...
class QuestionEdit(QuestionView):
    ...
    def set_connects(self):
        self.question.editingFiniashed.connect(self.save_question)
        self.answer.editingFinished.connect(self.save_answer)
        self.wrong_answer1.editingFinished.connect(self.wrong_answer1)
        self.wrong_answer2.editingFinished.connect(self.wrong_answer2)
        self.wrong_answer3.editingFinished.connect(self.wrong_answer3)
...
class QuestionListModel(QAbstractListModel):
    ...
    def random_AnswerCheck(self, list_model, w_question, widgets_list, w_showed_answer, w_result):
        frm = list_model.random_question()
        shuffle(widgets_list)
        frm_card = AnswerCheck(frm, w_question, widgets_list[0], widgets_list[1],
                               widgets_list[2], widgets_list[3], w_showed_answer, w_result)
        return frm_card
